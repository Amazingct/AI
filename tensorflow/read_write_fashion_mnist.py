...
for i in range(4):
    pic = test_images[i]
    pic1 = cv2.resize(pic,(600,600))
    info = "actual: " + class_name[test_labels[i]] + "  " + "prediction: " + class_name[np.argmax(prediction[i])]
    pic1 = cv2.putText(pic1,info,(10,10),cv2.FONT_HERSHEY_COMPLEX, 0.5, (255,0,0), 1)
    cv2.imshow("info", pic1)
    cv2.waitKey(1000)
    path = "/home/amazing/Desktop/PYTHON/AI/tensorflow/fashion" + "/" +  class_name[test_labels[i]] + "/" + str(i) + ".png"
    print(path)
