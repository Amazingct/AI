########################
# here we want to repeat the training process for 30 times and save the one with the highest accuracy
#######################
...
# train data using LinearRegression using the best fit line, (remember the usual linear graph)
# repeat process for 30 times and save the one with the highest accuracy
################################################
best = 0
for _ in range(30):
    x_train, x_test, y_train, y_test = sklearn.model_selection.train_test_split(x, y, test_size=0.1)
    ...
    # test the trained model and compare with actual result and return percentage of accuracy (acc)
    acc = linear.score(x_test, y_test)
    print(acc)

    if acc > best:
        best = acc
        # save the model (the generated formula etc) with the highest accuracy
        with open("studentModel.pickle", "wb") as f:
            pickle.dump(linear, f)

####################################################
print("chosen accuracy: ", best)
